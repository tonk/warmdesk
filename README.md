...
- **Command-line timer** — `warmdesk-timer start Website` when you begin, `warmdesk-timer stop` when you're done: the time is rounded up to whole 15 minutes, split at midnight, and booked as regular time entries on the chosen project; starting a new timer books the running one first, `stop --at 17:30` fixes a forgotten timer, and the timer lives on the server so it can be stopped from another machine. Installed by the Linux desktop `.deb`/`.rpm` packages, and ships with the server and as a standalone download for Linux, macOS, and Windows
